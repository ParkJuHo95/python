...
public class MainActivity04 extends AppCompatActivity {
    ...
    TextView tv1, tv2, tv3, tv4, tv5, tv6,test;
    Button btn;
...
//    }
    void lotto(){
        ArrayList happy = new ArrayList<Integer>();
        TextView[] views = {
                    tv1, tv2, tv3, tv4, tv5, tv6
        };
        for(int i=0; i<6;i++){
            int ran = (int)(Math.random()*45+1);
            if(happy.contains(ran)){
                i--;
            } else {
                happy.add(i, ran);
            }
        ...
        for(int i=0; i<6;i++) {
            views[i].setText(happy.get(i)+"");
        }
        test.append(Arrays.toString(happy.toArray()));
    }
}